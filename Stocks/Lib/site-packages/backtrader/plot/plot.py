...
class Plot_OldSync(with_metaclass(MetaParams, object)):
    ...
    def plot(self, strategy, figid=0, numfigs=1, iplot=True,
        ...
        for numfig in range(numfigs):
            ...
            fig.subplots_adjust(hspace=self.pinf.sch.plotdist,
                                top=0.98, left=0.12, bottom=0.07, right=0.95)
            ...
            self.mpyplot.autoscale(enable=True, axis=axtight, tight=False)
    ...
    def show(self):
        # self.mpyplot.figure(figsize=(16, 9))
        self.mpyplot.show()

    def savefig(self, fig, filename, width=32, height=18, dpi=1000, tight=True):
