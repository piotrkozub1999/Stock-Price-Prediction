...
class PInfo(object):
    ...
    def newfig(self, figid, numfig, mpyplot):
        fig = mpyplot.figure(figid + numfig, figsize=(13,9))
        # mpyplot.figure(figsize=(16,9))
...
class Plot_OldSync(with_metaclass(MetaParams, object)):
    ...
    def set_xmargin(self, ax, left, right):
        ax.set_xmargin(0)
        ax.autoscale_view()
        lim = ax.get_xlim()
        delta = np.diff(lim)
    ...
    def plot(self, strategy, figid=0, numfigs=1, iplot=True,
        ...
        for numfig in range(numfigs):
            ...
            fig.subplots_adjust(hspace=self.pinf.sch.plotdist,
                                top=0.98, left=0.15, bottom=0.07, right=0.95)
            ...
            # ax.margins(0.095)
            self.set_xmargin(ax, left=0.01, right=0.087)
        return figs
    ...
    def plotind(self, iref, ind,
        ...
        for lineidx in range(ind.size()):
            ...
            if indlabel == "Trades - Net Profit/Loss (True)":
                indlabel = "Trades - Profit/Loss"
        ...
        if not masterax:
            ...
            if self.pinf.sch.legendind and \
               ind.plotinfo._get('plotlegend', True):
                ...
                # labels[0] = labels[0].strip('Broker (None)\n')
                # labels[0] = labels[0].strip('Trades - Net Profit/Loss (True)\n')
        ...
        self.mpyplot.yticks(fontsize=13)
    ...
    def plotdata(self, data, indicators):
        ...
        if handles:
            ...
            # self.mpyplot.xticks(fontsize=13)
            # self.mpyplot.yticks(fontsize=13)
    ...
    def show(self):
        # self.savefig(fig=Plot, filename="Figure_10.png")
        self.mpyplot.show()


    def savefig(self, fig, filename, width=32, height=18, dpi=1000, tight=False):
